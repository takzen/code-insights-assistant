    ...
    if github_url:
        with st.spinner(f"Cloning repository from {github_url}..."):
            try:
                ...
                st.success(f"Repository successfully cloned.")
        ...
        if 'repo_path' in st.session_state:
            with st.spinner("Processing code and creating vector database... This may take a while for large repos."):
                try:
                    # Create vector store and save it in session state for reuse
...
# --- 2. Q&A Section ---
# This section only appears after the repo has been processed and the vector store exists
